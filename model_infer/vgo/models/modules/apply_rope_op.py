...
def _triton_rope_paper(
    ...
    # 2. cos and sin matrices are already in the shape (1, seq_len, head_dim // 2), so we simply load the entire matrix.  # noqa: E501
    ...
    even_k_offsets = tl.arange(0, pad_n_kh)[:, None] * hd + tl.arange(0, pad_hd // 2)[None, :] * 2
    even_q_mask = (tl.arange(0, pad_n_qh)[:, None] < n_qh) & (tl.arange(0, pad_hd // 2)[None, :] * 2 < hd)
    even_k_mask = (tl.arange(0, pad_n_kh)[:, None] < n_kh) & (tl.arange(0, pad_hd // 2)[None, :] * 2 < hd)
    q_tile_even = tl.load(q_ptr + even_q_offsets, mask=even_q_mask, other=0).to(sin_row.dtype)
    k_tile_even = tl.load(k_ptr + even_k_offsets, mask=even_k_mask, other=0).to(sin_row.dtype)

    # odd-indexed elements of the head
    odd_q_offsets = even_q_offsets + 1
    odd_k_offsets = even_k_offsets + 1
    odd_q_mask = (tl.arange(0, pad_n_qh)[:, None] < n_qh) & (tl.arange(0, pad_hd // 2)[None, :] * 2 + 1 < hd)
    odd_k_mask = (tl.arange(0, pad_n_kh)[:, None] < n_kh) & (tl.arange(0, pad_hd // 2)[None, :] * 2 + 1 < hd)
    q_tile_odd = tl.load(q_ptr + odd_q_offsets, mask=odd_q_mask, other=0).to(sin_row.dtype)
    k_tile_odd = tl.load(k_ptr + odd_k_offsets, mask=odd_k_mask, other=0).to(sin_row.dtype)
...
# Attention, q, k, cos, sin should be of the same dtype
def apply_rope(q, k, cos, sin, inplace=False):
    if not inplace:
        q = q.clone()
        k = k.clone()
    return LigerRopePaperFunction.apply(q, k, cos, sin)
