...
class QwenImageCausalConv3d(nn.Conv3d):
    ...
    def forward(self, x, cache_x=None):  # pyright: ignore[reportIncompatibleMethodOverride]
...
class QwenImageAttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # FIXME: not sure flash attention is called.
...
class AutoencoderKLQwenImage(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        base_dim: int = 96,
        z_dim: int = 16,
        dim_mult: tuple[int] = [1, 2, 4, 4], # type: ignore
        num_res_blocks: int = 2,
        attn_scales: list[float] = [],  # noqa: B006
        temperal_downsample: list[bool] = [False, True, True],  # noqa: B006
        dropout: float = 0.0,
        latents_mean: list[float] = [-0.7571, -0.7089, -0.9113, 0.1075, -0.1745, 0.9653, -0.1517, 1.5508, 0.4134, -0.0715, 0.5517, -0.3632, -0.1922, -0.9497, 0.2503, -0.2921],  # noqa: B006, E501
        latents_std: list[float] = [2.8184, 1.4541, 2.3275, 2.6558, 1.2196, 1.7708, 2.6052, 2.0743, 3.2687, 2.1526, 2.8652, 1.5579, 1.6382, 1.1253, 2.8251, 1.9160],  # noqa: B006, E501
    ) -> None:
    # fmt: on
        super().__init__()
        ...
        # When decoding spatially large video latents, the memory requirement is very high. By breaking the video latent  # noqa: E501
        # frames spatially into smaller tiles and performing multiple forward passes for decoding, and then blending the  # noqa: E501
    ...
    def _encode(self, x: torch.Tensor):
        ...
        for i in range(iter_):
            ...
            if i == 0:
                ...
                out = torch.cat([out, out_], 2) # type: ignore

        enc = self.quant_conv(out) # type: ignore
        self.clear_cache()
        return enc
    ...
    def _decode(self, z: torch.Tensor, return_dict: bool = True, enable_gradient_checkpointing: bool = False):
        ...
        for i in range(num_frame):
            ...
            if i == 0:
                ...
                if enable_gradient_checkpointing:
                    out = torch.utils.checkpoint.checkpoint(
                        self.decoder, x[:, :, i : i + 1, :, :], None, self._conv_idx
                    )
                else:
                    out_ = self.decoder(x[:, :, i : i + 1, :, :], feat_cache=self._feat_map, feat_idx=self._conv_idx)
                out = torch.cat([out, out_], 2) # type: ignore

        out = torch.clamp(out, min=-1.0, max=1.0) # type: ignore
        self.clear_cache()
        if not return_dict:
            return (out,)
    ...
    @apply_forward_hook
    def decode(self, z: torch.Tensor, return_dict: bool = True, enable_gradient_checkpointing: bool = False) -> Union[DecoderOutput, torch.Tensor]:  # noqa: E501
        r"""
        ...
        if self.use_slicing and z.shape[0] > 1:
            decoded_slices = [self._decode(z_slice, enable_gradient_checkpointing=enable_gradient_checkpointing).sample for z_slice in z.split(1)] # type: ignore  # noqa: E501
            decoded = torch.cat(decoded_slices)
        else:
            decoded = self._decode(z, enable_gradient_checkpointing=enable_gradient_checkpointing).sample # type: ignore
    ...
    def forward(
        ...
        """
        x = sample
        posterior = self.encode(x).latent_dist # type: ignore
